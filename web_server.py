...
# Model load state and lock to prevent double-loads
_models_lock = threading.Lock()
_models_loaded = False

def load_models():
    """
    Load model artifacts into globals. Safe to call multiple times; uses a lock.
    """
    global cnn_extractor, cnn_model, gru_model, label_encoder, metadata, _models_loaded
    ...
    with _models_lock:
        ...
        try:
            ...
            metadata_path = os.path.join(PROCESSED_DATA_PATH, "sequence_metadata.pkl")
            label_encoder_path = os.path.join(PROCESSED_DATA_PATH, "label_encoder.pkl")

            if not os.path.exists(metadata_path) or not os.path.exists(label_encoder_path):
                raise FileNotFoundError(f"Missing metadata or encoder in {PROCESSED_DATA_PATH}")
            ...
            # Load CNN model (handle .keras/.h5 naming)
            try:
            ...
            # Find a penultimate Dense layer of size 128 or fallback
            dense_layer = None
            ...
            if dense_layer is None:
                # fallback - pick third last layer (as original code did)
            ...
            # Keep models_unloaded so the app can return a 503 instead of crashing
            _models_loaded = False
...
def initialize_models():
    ...
    This runs once per worker (suitable for gunicorn).
    """
    try:
        load_models()
    except Exception:
        # If models fail to load, initialization will have logged the exception.
        # The analyze endpoint will return a 503 until the user fixes model artifacts.
        logger.error("Model initialization failed at startup; endpoints will return 503 until fixed.")
...
def analyze():
    global _models_loaded, metadata, label_encoder, cnn_extractor, cnn_model, gru_model

    if not _models_loaded:
        return jsonify({"error": "Models are not loaded yet. Please try again later."}), 503
    ...
    try:
        # Load parameters from metadata (with safe defaults)
        ...
        pred_class_idx = int(np.argmax(gru_probs))
        ...
        for i in range(total_chunks):
            ...
            chunk_class_idx = int(np.argmax(chunk_prob))
        ...
        logger.exception("Failed to analyze audio: %s", e)
        return jsonify({"error": f"Failed to analyze audio: {str(e)}"}), 500

if __name__ == '__main__':
    # Local dev: try to load models synchronously so local runs behave like production.
    try:
        load_models()
    except Exception:
        logger.warning("Models failed to load during local startup; continue to run so you can inspect logs.")

    port = int(os.environ.get("PORT", 5000))
    # Bind to 0.0.0.0 so the container exposes the port correctly
